    ...
    return (
        <form onSubmit={handleSubmit} className={cn("flex gap-4 items-start", className)}>
            <div className="w-10 h-10 rounded-full bg-accent-primary flex items-center justify-center shrink-0 shadow-sm border border-accent-primary/20">
                <span className="text-background font-bold text-sm">Me</span>
            </div>
            <div className="flex-1 flex flex-col gap-3">
                <textarea
                    ...
                    className="w-full min-h-[80px] bg-surface border border-border-subtle rounded-xl px-4 py-3 text-sm resize-y focus:outline-none focus:border-accent-primary transition-all focus:ring-2 focus:ring-accent-primary/10"
                    onKeyDown={(e) => {
                    ...
                    <button
                        ...
                        className={cn(
                            ...
                            text.trim() && !isSubmitting
                                ? "bg-accent-primary text-background hover:bg-accent-hover hover:-translate-y-0.5 active:translate-y-0 shadow-lg shadow-accent-primary/30"
                                : "bg-surface-sunken text-text-muted cursor-not-allowed border border-border-subtle"
                        )}
                    >
