 ...
 *
 * @param {{ submissions: Array }} props
 */
export const ProblemSubmissions = ({ submissions = [] }) => {
    const verdictStyles = {
        ...
        <div className="flex flex-col gap-1 p-4 overflow-y-auto h-full custom-scrollbar">
            {submissions.map((sub, i) => {
                ...
                return (
                    <div
                        key={sub.id || i}
                        className="flex items-center justify-between p-3 rounded-xl hover:bg-white/5 border border-transparent hover:border-white/5 transition-all text-sm"
                    >
