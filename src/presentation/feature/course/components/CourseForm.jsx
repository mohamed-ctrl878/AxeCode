 ...
 */
export const CourseForm = ({ initialData = {}, isLoading = false, onSubmit }) => {
    // Form State
    const [title, setTitle] = useState(initialData.title || '');
    const [difficulty, setDifficulty] = useState(initialData.difficulty || 'Easy');
    const [description, setDescription] = useState(initialData.description || []);
    
    // Arrays & Relations (CSV text inputs for MVP)
    const [tagsInput, setTagsInput] = useState((initialData.tags || []).join(', '));
    const [courseTypesInput, setCourseTypesInput] = useState((initialData.course_types || []).join(', '));
    const [problemTypesInput, setProblemTypesInput] = useState((initialData.problem_types || []).join(', '));
    ...
    const handleSubmit = (e) => {
        ...
        const courseTypeIds = courseTypesInput.split(',').map(s => parseInt(s.trim(), 10)).filter(n => !isNaN(n));
        const problemTypeIds = problemTypesInput.split(',').map(s => parseInt(s.trim(), 10)).filter(n => !isNaN(n));
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6 w-full mx-auto p-6 lg:p-10 bg-surface rounded-2xl border border-white/5 shadow-2xl">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div className="flex flex-col gap-5">
                    ...
                    {/* Course Types */}
                    <div className="flex flex-col gap-2">
                        <label className="text-sm font-bold text-text-muted">Categorization IDs (CSV)</label>
                        <input
                            type="text"
                            value={courseTypesInput}
                            onChange={(e) => setCourseTypesInput(e.target.value)}
                            placeholder="Course Type IDs e.g. 1, 4"
                            className="bg-background border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-accent-primary transition-colors"
                        />
                        <input
                            type="text"
                            value={problemTypesInput}
                            onChange={(e) => setProblemTypesInput(e.target.value)}
                            placeholder="Problem Type IDs e.g. 2, 5"
                            className="bg-background border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-accent-primary transition-colors"
                        />
                    </div>
