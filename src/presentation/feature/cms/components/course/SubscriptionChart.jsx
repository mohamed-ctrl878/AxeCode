 ...
 */
export const SubscriptionChart = ({ data = [], filter = 'all' }) => {
  ...
  const chartData = useMemo(() => {
    ...
    return interval.map(date => {
      const count = data.filter(entry => {
        ...
        return isAfter(entry.startDate, date) && isAfter(addDays(date, 7), entry.startDate);
      }).length;
  ...
  return (
    <div className="relative w-full bg-surface-dark/20 border border-white/5 rounded-[2rem] p-8 backdrop-blur-md overflow-hidden group/chart animation-fade-in">
      {/* Glossy Overlay */}
      ...
      <div className="flex items-center justify-between mb-8 relative z-10">
        ...
        <div className="flex items-baseline gap-1">
          ...
          <span className="text-[10px] font-bold text-text-muted uppercase tracking-tighter">Total Students</span>
        </div>
      </div>
      ...
      <div className="relative h-[240px] w-full">
        ...
        {hoveredPoint && (
          ...
          >
            ...
            <div className="w-3 h-3 bg-surface-dark border-r border-b border-white/10 rotate-45 mx-auto -mt-1.5" />
          </div>
        )}
      </div>
    </div>
