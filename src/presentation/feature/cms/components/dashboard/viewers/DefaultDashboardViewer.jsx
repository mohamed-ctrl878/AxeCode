    ...
    return (
        <div className="animation-fade-in">
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h2 className="text-sm font-serif font-bold text-text-primary uppercase tracking-widest">Platform Activity Trend</h2>
                ...
                <div className="flex items-center gap-2 text-[10px] bg-text-primary/5 px-2 py-1 rounded-full border border-text-primary/10">
                    <span className="text-text-primary font-bold font-mono uppercase tracking-tighter">LIVE_AUDIT</span>
                </div>
            </div>
            ...
            <div className="h-64 flex items-end justify-between gap-[2px] mt-8 border-b border-border-subtle pb-4 relative">
                ...
                {aggregateTimeline.length > 0 ? aggregateTimeline.map((item, i) => {
                    ...
                    return (
                        <div 
                            key={item.time} 
                            className="flex-1 bg-text-primary/20 rounded-t-[1px] relative group hover:bg-text-primary transition-all duration-300" 
                            style={{ height: `${Math.max(height, 2)}%`, minWidth: '4px' }}
                        >
                            <div className="absolute -top-12 left-1/2 transform -translate-x-1/2 bg-text-primary text-surface text-[9px] py-1.5 px-3 rounded opacity-0 group-hover:opacity-100 transition-opacity shadow-lg z-20 pointer-events-none whitespace-nowrap border border-ivory/20">
            ...
            <div className="mt-8 grid grid-cols-2 gap-4">
                <div className="p-4 bg-surface-sunken/50 rounded-2xl border border-border-subtle">
                    ...
                    <span className="text-2xl font-serif font-bold text-text-primary">{maxVal === 1 ? 0 : maxVal}</span>
                </div>
                <div className="p-4 bg-surface-sunken/50 rounded-2xl border border-border-subtle">
                    <span className="text-[9px] text-text-muted uppercase font-bold tracking-widest block mb-1">Session Actions</span>
                    <span className="text-2xl font-serif font-bold text-text-primary">{aggregateTimeline.reduce((acc, curr) => acc + curr.count, 0)}</span>
                </div>
            </div>
        </div>
    );
