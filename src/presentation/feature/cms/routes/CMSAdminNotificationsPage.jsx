        ...
        <div className="space-y-8 animation-fade-in">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 bg-surface p-6 md:p-8 rounded-3xl md:rounded-[32px] border border-border-default shadow-whisper">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        <h2 className="text-2xl md:text-3xl font-serif font-bold tracking-tight text-text-primary">Admin Alerts</h2>
                    ...
                    return (
                        <button
                            ...
                            className={cn(
                                ...
                                isActive ? "bg-text-primary text-surface shadow-lg" : "text-text-muted hover:bg-surface-sunken hover:text-text-primary"
                            )}
                        >
                            <TabIcon size={14} />
                            {tab.label}
            ...
            <div className="bg-surface border border-border-default rounded-[32px] shadow-whisper overflow-hidden">
                <div className="bg-background/80 px-4 md:px-8 py-3 md:py-4 border-b border-border-default">
                    <span className="text-[11px] font-bold text-text-primary/50 font-serif uppercase tracking-[0.3em]">Admin Notification Feed</span>
                </div>
                ...
                <div className="divide-y divide-border-subtle/50 min-h-[300px]">
                    {isLoading ? (
                        <div className="flex flex-col items-center justify-center py-32 gap-4 animate-pulse">
                            ...
                            <p className="text-[11px] font-bold text-text-primary/40 font-serif uppercase tracking-[0.3em]">Loading alerts...</p>
                        </div>
                    ) : (!Array.isArray(notifications) || notifications.length === 0) ? (
                        <div className="flex flex-col items-center justify-center py-32 gap-6 opacity-40">
                            <Database size={60} className="text-text-muted" />
                            <p className="text-lg font-serif font-bold text-text-primary">No Alerts</p>
                        ...
                        return (
                            <div key={id} className={cn("px-4 md:px-8 py-4 md:py-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 hover:bg-background/40 transition-all group", !isRead && "bg-amber-50/30")}>
                                <div className="flex items-start md:items-center gap-4 md:gap-5 w-full md:w-auto flex-1 min-w-0">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <div className="text-sm font-serif font-bold text-text-primary break-words md:truncate">{message}</div>
