            ...
            <div className="w-full max-w-4xl flex flex-col w-full">
                <div className="flex items-center justify-between mb-10">
                    <div className="flex items-center gap-6">
                        <button 
                            ...
                            className="w-12 h-12 rounded-2xl bg-surface border border-border-subtle flex items-center justify-center text-text-muted hover:border-text-primary hover:text-text-primary transition-all group shadow-sm"
                        >
                    ...
                    <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-surface border border-border-default text-[10px] font-mono text-text-primary/60 shadow-inner">
                        <Flag size={14} className="text-accent-primary" />
                        <span>Archive Entry</span>
                    </div>
                </div>

                <form onSubmit={handleSave} className="w-full space-y-8 bg-surface p-10 rounded-[40px] border border-border-default shadow-whisper">
                    <div className="space-y-4">
                    <label className="text-[11px] font-bold text-text-primary/50 font-serif uppercase tracking-[0.3em] px-1">
                        Reason Descriptor
                    </label>
                    <input 
                        type="text"
                        ...
                        required
                        className="w-full h-16 bg-surface-sunken border border-border-default rounded-2xl px-6 text-text-primary font-serif text-lg focus:border-text-primary focus:ring-4 focus:ring-near-black/5 outline-none transition-all placeholder:text-text-muted/40"
                    />
                </div>
                ...
                <div className="flex items-center justify-end gap-6 pt-6 border-t border-border-default">
                    <button 
                        type="button"
                        onClick={() => navigate(PATHS.CMS_REPORT_TYPES)}
                        className="text-[12px] font-serif font-bold tracking-widest text-text-muted hover:text-text-primary uppercase transition-colors"
                    >
                        Discard Changes
                    </button>
